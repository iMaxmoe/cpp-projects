...
private:
    ...
    ChessPiece* chessBoard[8][8]; // The internal ChessBoard. Passed to all ChessPieces.First row then column
	King* const whiteKing;
	King* const blackKing;
    Pawn* pawnFirstMove; // For En Passant Timing Rules. (only one possible pawnFirstMove lol)
	ChessPiece* selectedChessPiece;
    bool selectedChessPieceValidMoves[8][8]; // good design: no need to create potentialMove arr for every piece, just update it.
	Player currentPlayer;

	// ChessBoard State.
    King* getPlayerKing(Player player) const;
    bool hasValidMoves(Player player) const; // bool selectedChessPieceValidMoves[8][8] all false?
